          ...
          {navItems.map((item) =>
            item.dropdown ? (
              <div key={item.label} className="relative group">
                {/* Products button */}
                <button
                  className={`relative text-lg font-semibold tracking-wide flex items-center gap-1 text-gray-800 transition-colors duration-300 hover:text-purple-700 ${active === item.label ? "text-purple-700" : ""
                    }`}
                ...
                >
                  ...
                  <ChevronDown
                    size={14}
                    strokeWidth={4}
                    className={`ml-1 font-semibold transition-transform duration-300 ${dropOpen ? "rotate-180" : "rotate-0"
                      } group-hover:rotate-180`}
                  />
                  {/* Underline */}
                  <span
                    className={`absolute left-0 -bottom-1 h-[2px] bg-gradient-to-r from-purple-500 to-purple-700 transition-all duration-300 ${active === item.label || dropOpen ? "w-full" : "w-0 group-hover:w-full"
                      }`}
                  />
                </button>
              ...
              <Link
                ...
                className={`relative  flex items-center justify-center gap-1 text-lg font-semibold tracking-wide group transition-all duration-300 ${active === item.label ? "text-purple-700" : "text-gray-800"
              ...
              >
                ...
                {/* underline */}
                <span
                  className={`absolute left-0 -bottom-1 h-[2px] bg-gradient-to-r from-purple-500 to-purple-700 transition-all duration-300 ${active === item.label ? "w-full" : "w-0 group-hover:w-full"
                    }`}
                />
              </Link>
            )
        ...
        <div className="lg:hidden bg-[#1e0f44] px-4 py-4 space-y-2">
              ...
              >
                ...
                {item.dropdown && (
                  <ChevronDown
                    size={16}
                    strokeWidth={3}
                    className={`transition-transform duration-300 ${dropOpen && active === item.label ? "rotate-180" : "rotate-0"
                      }`}
                  />
                )}
              </button>
          ...
          <div className="flex items-center justify-between gap-2 mt-4">
            ...
            >
              <Heart
                size={20}
                className={`transition-all duration-300 fill-white stroke-white ${donateClicked
